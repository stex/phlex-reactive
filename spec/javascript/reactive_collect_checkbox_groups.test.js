...
// group is not expressed by any of them: it is announced in `empty_groups[]`.
...
test("an empty group plus a file input is ANNOUNCED, not silently dropped", async () => {
  // A form body cannot carry an empty array. The group's key stays absent and
  // its name rides in `empty_groups[]`, so the server can fill the absence with
  // [] — the same value the JSON path sends outright. Announcing it rather than
  // sending a blank keeps `[""]` meaning what it means: `[:date]` and `[:file]`
  // read it as "did not come in".
  const root = new FakeNode({ tag: "div", controller: "reactive" })
  root.append(
    checkbox("features[]", "news", false),
    checkbox("features[]", "events", false),
  ...
  expect(captured.body instanceof FormData).toBe(true)
  expect(captured.body.getAll("params[features][]")).toEqual([])
  expect(captured.body.getAll("empty_groups[]")).toEqual(["features"])
})

test("a group WITH values is not announced as empty", async () => {
  const root = new FakeNode({ tag: "div", controller: "reactive" })
  root.append(
    checkbox("features[]", "news", true),
    new FakeNode({
  ...
  expect(captured.body.getAll("params[features][]")).toEqual(["news"])
