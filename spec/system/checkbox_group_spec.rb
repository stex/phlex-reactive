...
RSpec.describe "Reactive checkbox group (issue #258)", type: :system do
  ...
  it "clears a group over the FORM-encoded path, end to end (issue #258)" do
    # With a file attached the client sends FormData, where an empty array is
    # not expressible: the group's key is left out and its name is announced in
    # `empty_groups[]`. This is the only test that drives the client's name
    # format through the server's resolver — the two halves otherwise each
    # assert their own expectation of the other.
    visit "/checkbox_group"
